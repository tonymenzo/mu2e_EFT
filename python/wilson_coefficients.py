...
class WC_3_flavor:
    ...
    def match_nucleon_basis(self, p1, q):
        """
        Match the 3-flavor parton basis to the nucleon basis from 2208.07945

        p1 (numpy array): Incoming 4-momentum of the muon
        ...
        ######################################
        # NUMERICAL VALUE NEEDS TO BE CHOSEN #
        ######################################
        mL = 1.0
    ...
    def get_isoscalar_isovector(self, p1, q):
        """
        ...
        wcN = self.match_nucleon_basis(p1, q)
    ...
    def write_yaml(self, p1, q, element, isotope, interaction, oscb, isochar, plots, mL, compute_rate = False, path = './wcN.yaml'):
        """
        ...
        wcN = self.get_isoscalar_isovector(p1,q)
